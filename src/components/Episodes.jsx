...
function Episodes() {
  ...
  useEffect(() => {
    findShow(id);
    console.log(
      "these are shows details and episoes",
      show.episodes && show.episodes.items.map((item) => item.name)
    );
    ...
    <div>
      <div className="bg-base-100 py-24 flex flex-row flex-wrap justify-center gap-9 text-3xl p-8 ">
        <img
          ...
          src={show.images && show.images[1].url}
          alt=""
        />
        <div className=" self-center flex flex-col">
          <h1 className=" font-bold text-5xl">{show.name}</h1>
          <p className=" pt-10 text-xl lg:text-left  font-medium">
            {show.publisher}
          </p>

          <p className=" max-w-xl text-base rounded-2xl mt-2  py-3  ">
            {show.description}
          </p>
        </div>
      </div>
              ...
              <>
                <div className=" hover:bg-slate-700 max-w-lg transition-all card lg:max-w-5xl lg:card-side bg-slate-800 shadow-2xl ">
                  <figure>
                    <img
                      width={350}
                      className=" pt-8 lg:p-0 shadow-xl p-2 lg:w-64"
                  ...
                  <div className="card-body px-16 py-7  max-w-xl lg:max-w-max">
                    ...
                    <p className="w-full">{`${item.description.slice(
                      0,
                      200
                    )} ....`}</p>
                    <div className="card-actions justify-end py-1">
                      <div className=" btn btn-ghost mr-16">
                        {msConversion(item.duration_ms)}
                      </div>
